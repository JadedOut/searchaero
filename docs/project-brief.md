...
4. **Notification delivery is the agent's job, not searchaero's.** searchaero finds deals and returns structured data. The calling agent delivers notifications through whatever channel the user has available — Gmail MCP (already OAuth'd, zero setup), ntfy (user choice for self-hosted push), Slack, etc. searchaero includes a built-in `send_ntfy()` helper as a convenience for users who choose ntfy, but it is not a fallback or default — it's one option among many. The design principle: searchaero should never need to know *how* you want to be notified. If the agent has access to a Gmail MCP server, it can send email without searchaero touching SMTP. If the user prefers ntfy, they configure a topic and searchaero sends directly. The watchlist system (`searchaero watch`) combines route monitoring, scheduled scraping, and condition evaluation into an automated pipeline — notification delivery plugs in at the end via the agent or the user's chosen channel.

5. **No agent instructions in config files.** Agent discoverability happens through MCP tool schemas and `searchaero schema`, not by embedding CLI manuals in agent-specific config files (CLAUDE.md, .cursorrules, etc.). The tool describes itself; agents don't need a cheat sheet.
...
searchaero watch setup --ntfy-topic my-topic     # configure ntfy.sh push notifications
```
...
2. **Direct ntfy** — User configures an ntfy topic (`searchaero watch setup`); searchaero sends HTTP POST directly via stdlib `urllib.request`. Works headless (cron, daemon) with no agent in the loop. Config in `~/.searchaero/config.json` with env var overrides (`SEARCHAERO_NTFY_TOPIC`, `SEARCHAERO_NTFY_SERVER`).
...
searchaero watch setup --ntfy-topic my-secret-topic # configure ntfy notifications
searchaero status                                   # DB stats, coverage, freshness
searchaero setup                                    # init DB schema, check credentials
searchaero schedule add daily-run --every daily --file routes/canada_us_all.txt  # schedule a job
searchaero schedule list                            # show scheduled jobs
searchaero schedule remove daily-run                # delete a schedule
searchaero schedule run                             # start scheduler (foreground)
...
AI Agent  ←JSON-RPC→  mcp_server.py  ←import→  core/db.py
                            ...
                            │
                            └──file handoff──→  ~/.searchaero/mfa_request  (MFA needed)
                                                ~/.searchaero/mfa_response (code from submit_mfa)
```
...
4. Agent calls `submit_mfa("847291")` → writes code to `~/.searchaero/mfa_response`, waits for scrape thread to finish
...
Any MCP-compatible client discovers typed tool schemas automatically — parameter names, types, descriptions, and return formats. No manual instructions needed. Register in Claude Code with `claude mcp add searchaero -- searchaero-mcp`, or add to `.mcp.json` for project-scoped auto-discovery.

For agents without MCP support, `searchaero schema` returns the same information as JSON, and all commands support `--json` for structured output.
...
- **Scheduling** — `searchaero schedule add/list/remove/run` for built-in cron-based scheduling. APScheduler 3.x with SQLAlchemyJobStore persists jobs to `~/.searchaero/schedules.db`. Human-friendly aliases (`daily`, `hourly`, `twice-daily`). Each job runs `searchaero search --file <routes> --headless --create-schema` then `searchaero alert check`. `schedule run` blocks in foreground; OS-level daemonization left to user. `core/scheduler.py`
...
| **10** | **~~Schedule, visualization, agent hints.~~** Done. `searchaero schedule add/list/remove/run` with APScheduler 3.x + SQLite persistence. Rich-powered colored tables with inline Unicode sparklines for price trends. `searchaero schema [command]` for runtime introspection, `--meta` for field type hints, `--fields` for JSON field selection, structured error JSON. `core/output.py`, `core/schema.py`, `core/scheduler.py`. 282 tests passing (49 new). | ~~CLI needs scheduling, visual output, and agent discoverability.~~ |

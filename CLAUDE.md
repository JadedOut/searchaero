...
- `core/aeroplan_runner.py` — `run_aeroplan_route_with_reauth`: bounded re-auth-and-resume loop that survives the ~30–40 min session TTL. On expiry with windows remaining it waits a log-normal cooldown, re-authenticates, and resumes from the next unscraped window (capped by `max_reauths`=2 + `deadline_seconds`, CLI default 1800 s); returns a `stop_reason` (`complete`/`deadline`/`max_reauths`); wired into `cli.py::_scrape_route_aeroplan_live`
- `core/humanize.py` — log-normal timing keystone (`human_delay`/`human_sleep`, mean-preserving, clamped, injectable RNG). Drives the re-auth cooldown, login-step settles, per-key credential/2FA typing, and the scraper's inter-window jitter — replaces uniform `random.uniform()` jitter (the "jitter trap"). See `docs/findings/aeroplan/phase-3-unattended.md` → Login-hardening
